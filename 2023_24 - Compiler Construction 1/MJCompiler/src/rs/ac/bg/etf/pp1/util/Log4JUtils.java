...
public class Log4JUtils {
    ...
    public void prepareLogFile(final Logger root) {
        final Appender appender = root.getAppender("file");

        if (!(appender instanceof FileAppender))
            return;
        final FileAppender fAppender = (FileAppender) appender;

        String logFileName = fAppender.getFile();
        logFileName = logFileName.substring(0, logFileName.lastIndexOf('.')) + "-test.log";

        final File logFile = new File(logFileName);
        final File renamedFile = new File(logFile.getAbsoluteFile() + "." + System.currentTimeMillis());

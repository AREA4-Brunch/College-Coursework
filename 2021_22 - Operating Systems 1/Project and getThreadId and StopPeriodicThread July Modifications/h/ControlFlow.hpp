...
class ControlFlow
{
 ...
 public:
    ...
    void waitForPeriodicThreads();

    void increasePeriodicThreadCount(::PeriodicThread *thread);

    void decreasePeriodicThreadCount(::PeriodicThread *thread);

    void signalKillOldestPeriodicThread();

    void lockPeriodicThread();

    void unlockPeriodicThread();
...
//    ~ControlFlow();
    ...
    kernel::Semaphore *periodic_thread_counter_mutex = nullptr;

    int periodic_thread_counter = 0;

    kernel::PCB *periodic_thread_killer = nullptr;

    kernel::Semaphore *periodic_thread_killer_sem = nullptr;

    // sorted by age since constructor is called,
    // from oldest to newest
    kernel::utils::LinkedList<::PeriodicThread*> *periodic_threads = nullptr;
    ...
    void setPeriodicThreadKiller(kernel::PCB *periodic_thread_killer);
    ...
    static void killOldestPeriodicThread(void *);
